...
export class UserServiceProvider {
  ...
  public getUser(token: String) {
    let url = "http://192.168.1.95:8000/api/profil?token="+token;
    console.log(url);
    return this.http.get(url, { headers: this._headers })
  }
  public updateUser(user:Object,token:string){
    // check if all required fields are there
    let url = "http://192.168.1.95:8000/api/profil/update";
  ...
  public updateSecurity(passwords,token){
    let url = "http://192.168.1.95:8000/api/profil/password/update";
    const body = new HttpParams()
      .set('token', token)
  ...
  public Desabon_commune(id_commune,token){
    let url = "http://192.168.1.95:8000/api/commune/delete/"+id_commune;
